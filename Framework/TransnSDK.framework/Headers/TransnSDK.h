...
@property (nonatomic, copy, readonly) ChatOrderManger   *chatOrderManger;

@property (nonatomic, strong) ReservationManager *reservationManager;
 ...
 *   @param mode 调用模式
 */
+ (void)registerAppKey:(NSString *)appKey appSecret:(NSString *)appSecret andDebugmode:(TransnDebugMode)mode;
...
/*!
 ...
 @param block 消息数组
 */
- (void)selectMessagesByUserId:(NSString *)userId result:(void (^)(NSArray <TransnMessage *> *messages))block;
...
/*!
 ...
 @param block 消息数组
 */
- (void)selectMessagesByUserId:(NSString *)userId translatorId:(NSString *)translatorId result:(void (^)(NSArray <TransnMessage *> *messages))block;
