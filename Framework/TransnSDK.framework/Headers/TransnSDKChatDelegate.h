 ...
 *   @param image 上传的图片
 */
- (void)sendImageMessage:(TransnMessage *)message didCompleteUpdateImage:(UIImage *)image;
 ...
 *   @param message 消息体
 */
- (void)onRecvMessages:(TransnMessage *)message;
...
/// 网络状态.2秒返回一次
//- (void)onCallNetStatus:(NetCallNetStatus)status TRAPIDeprecated("加了前缀");

/// 网络状态.2秒返回一次
- (void)onCallNetStatus:(TRNetCallDisconnectReason)status TRAPIDeprecated("加了前缀");
 ...
 */
- (void)receiveTransnerName:(NSString *)username image:(NSString *)image translatorId:(NSString *)translatorId translatorRemark:(NSString *)translatorRemark;// TRAPIDeprecated("使用新方法 -(void)onReceiveTranslatorModel:(TransnTranslatorModel *)Translator");
 ...
 *   },
 */
- (void)onRecvSensitiveWordsMessage:(TransnMessage *)message;
...
/// 收到TransnBox Tips消息
- (void)onRecvTransnBoxTipsMessages:(TransnMessage *)message;
